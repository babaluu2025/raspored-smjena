...
// ------------------------------------------------------------
//  KOMPONENTA ZA TABELU (IZDVOJENA – NE GUBI FOKUS!)
// ------------------------------------------------------------
const SektorTabela = ({ sektor, radnici, naslov, onAzurirajIme, onAzurirajSmjenu, onAzurirajDan, onAzurirajPauzu }) => {
  return (
    <div className="mb-3 print:mb-2">
      ...
      <div className="overflow-x-auto -mx-1 px-1">  {/* horizontalno skrolovanje na mobilnom */}
              ...
              <tr key={index}>
                ...
                <td className="border border-black p-1 text-center bg-white">
                  <select
                    ...
                    style={{ fontSize: '10px', backgroundColor: radnik.smjena === 'I' ? '#DBEAFE' : '#DCFCE7' }}
                  >
                ...
                {DANI.map(dan => {
                  ...
                  let bg = '#FFFFFF';
                  if (val === 'OFF') bg = '#FEE2E2';
                  else if (val === 'I') bg = '#DBEAFE';
                  else if (val === 'II') bg = '#DCFCE7';
                  return (
                    <td key={dan} className="border border-black p-1 text-center" style={{ backgroundColor: bg }}>
                      <select
                        value={val}
                        ...
                        style={{ fontSize: '10px' }}
                      >
                        {SMJENE.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </td>
                  );
                })}
...
export default function RasporedApp() {
  ...
  // Handleri (stabilne reference sa useCallback)
    ...
    <div className="min-h-screen bg-gray-100 print:bg-white">
      ...
      <style>{`
        @media print {
          ...
          input, select {
            ...
            -webkit-appearance: none; font-size: 11px !important; font-weight: bold !important; color: black !important;
          }
      ...
      <div className="bg-white shadow-lg mb-4 no-print">
        <div className="max-w-full mx-auto p-3 sm:p-4">
          <div className="flex flex-wrap justify-between items-center gap-3 mb-3">
            ...
            <p className="text-xs sm:text-sm text-gray-500">I: 08-16h | II: 16-00h</p>
          </div>

          <div className="flex flex-wrap items-center gap-2 mb-3">
            <button onClick={() => promeniSedmicu(-1)} className="bg-gray-600 text-white px-3 py-1.5 rounded text-sm">◀</button>
            <div className="bg-white border px-3 py-1.5 rounded font-bold text-sm">{datumPrikaz}</div>
            <button onClick={() => promeniSedmicu(1)} className="bg-gray-600 text-white px-3 py-1.5 rounded text-sm">▶</button>
          </div>

          <div className="flex flex-wrap gap-2">
            <button onClick={() => dodajRadnika('KONOBARI')} className="bg-green-600 text-white px-3 py-1.5 rounded text-xs sm:text-sm">+ Konobar</button>
            <button onClick={() => dodajRadnika('KUHINJA')} className="bg-green-600 text-white px-3 py-1.5 rounded text-xs sm:text-sm">+ Kuvar</button>
            <button onClick={() => dodajRadnika('ŠANK')} className="bg-green-600 text-white px-3 py-1.5 rounded text-xs sm:text-sm">+ Šanker</button>
            <button onClick={autoRasporedi} className="bg-blue-600 text-white px-3 py-1.5 rounded text-xs sm:text-sm">🔄 Auto</button>
            <button onClick={resetujPodatke} className="bg-gray-600 text-white px-3 py-1.5 rounded text-xs sm:text-sm">🔃 Reset</button>
            <button onClick={() => window.print()} className="bg-black text-white px-4 py-1.5 rounded text-xs sm:text-sm font-bold">🖨️ Štampaj</button>
          </div>
        </div>
      </div>

      {/* SADRŽAJ (vidljiv i na ekranu i u štampi) */}
      <div className="max-w-full mx-auto px-2 sm:px-4 print:px-0">
        <div className="bg-white rounded-lg shadow print:shadow-none p-2 sm:p-4 print:p-0">
          <div className="hidden print:block text-center mb-2">
            <h1 style={{fontSize:'14px',fontWeight:'bold'}}>RASPORED SMJENA</h1>
            <p style={{fontSize:'11px',fontWeight:'bold'}}>{datumPrikaz}</p>
          </div>
      ...
      <div className="max-w-full mx-auto px-2 sm:px-4 mt-4 no-print">
        <div className="bg-white rounded-lg shadow p-3 text-sm">
          ...
          <span className="inline-block w-3 h-3 bg-blue-200 border border-black ml-2 mr-1"></span> I (08-16h)
          <span className="inline-block w-3 h-3 bg-green-200 border border-black ml-2 mr-1"></span> II (16-00h)
          <span className="inline-block w-3 h-3 bg-red-200 border border-black ml-2 mr-1"></span> OFF
        </div>
      </div>
    </div>
  );
